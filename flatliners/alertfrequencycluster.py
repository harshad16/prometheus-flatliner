...
class AlertFrequencyCluster(BaseFlatliner):
    ...
    def calculate_frequency(self, alert_name, cluster_id, time_window, time_stamp):
        ...
        self.clusters[cluster_id][alert_name].time_stamps = [
            x
            for x in self.clusters[cluster_id][alert_name].time_stamps
        ...
        self.clusters[cluster_id][alert_name].frequency = float(
            len(self.clusters[cluster_id][alert_name].time_stamps)
        )
    ...
    def normalize_cluster(self, cluster_id, alert):
        ...
        for value, name in zip(alert_list, alert_names):
            if max_value != min_value:
                self.clusters[cluster_id][name].frequency = (
