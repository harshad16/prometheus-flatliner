...
class InfluxdbStorage(BaseFlatliner):
    ...
    def on_next(self, x):
            ...
            self.buffer_list.append(
                {
                    ...
                    "tags": {"clusterID": x.cluster, "clusterVersion": x.version},
                    "time": datetime.utcfromtimestamp(x.timestamp).strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    ),
                    "fields": {"weirdness_score": x.weirdness_score},
                }
            )
            ...
            self.buffer_list.append(
                {
                    ...
                    "fields": {"resource_deltas": x.resource_deltas[x.resource]},
                }
            )
            ...
            self.buffer_list.append(
                {
                    ...
                    "fields": {"alert_weirdness_score": x.weirdness_score},
                }
            )
            ...
            self.buffer_list.append(
                {
                    ...
                    "fields": {"alert_deltas": x.alert_deltas[x.alert]},
                }
            )
        ...
        if len(self.buffer_list) > self.buffer_size:
            try:
                ...
                _LOGGER.exception(
                    "Failed to flush Influx Buffer. Buffer size:{0}".format(
    ...
    def flush_buffer(self):
        _LOGGER.debug(
            "Flushing Influx buffer data to the DB, buffer size:{0}".format(
                len(self.buffer_list)
            )
        )
