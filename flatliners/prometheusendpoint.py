...
class PrometheusEndpoint(BaseFlatliner):
    def __init__(self, pruning_interval: int = 300):
        ...
        _LOGGER.info(
            "Prometheus Endpoint initialized. Metric pruning interval is {0} seconds".format(
    ...
    def on_next(self, x):
        try:
            ...
            self.weirdness_score_gauge.labels(
                cluster=str(x.cluster), version=str(x.version)
            ).set(x.weirdness_score)

            # Store timestamp when the metric was published and metric version info
            self.published_metric_timestamps[str(x.cluster)] = [
                int(time()),
                str(x.version),
            ]
        except Exception as e:
            _LOGGER.error(
                "Couldn't process the following packet {0}. Reason: {1}".format(
                    x, str(e)
                )
            )
            raise e
    ...
    def _delete_stale_metrics(self):
        """The function will remove any metric that was published $(pruning_interval) seconds ago or older."""
        ...
        for cluster_id in list(self.published_metric_timestamps):
            if self.published_metric_timestamps[cluster_id][0] < timestamp_threshold:
                ...
                self.weirdness_score_gauge.remove(
                    cluster_id, self.published_metric_timestamps[cluster_id][1]
                )
    ...
    def start_server(self):
        ...
        while True:
            ...
            _LOGGER.debug(
                "Next metric pruning will be in {} seconds".format(
                    self.pruning_interval
                )
            )
